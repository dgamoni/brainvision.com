...
function custom_child_scripts() {
	...
	if ( is_page_template('template-vendors.php')  ) {
		wp_enqueue_style(
			'vendor_template_style', 
			CORE_URL . '/css/vendor_template_style.css',
			array(),
			rand()
		);		
		wp_enqueue_script(
		    'vendor_template_js',
		    CORE_URL . '/js/vendor_template.js',
	        array('jquery'), 
	        rand(),
	        true  
		);
		wp_localize_script( 'vendor_template_js', 'js_url', 
            array( 
                'imgurl' => CORE_URL.'/img/',
